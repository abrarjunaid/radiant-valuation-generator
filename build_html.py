...
def build_html(data: dict, agent_name: str, insights: list) -> str:
    ...
    is_rent = data.get('report_type') == 'rent'
    val = data.get('value')
    annual_rent = data.get('annual_rent')
    psf = data.get('price_per_sqft')
    change_str = data.get('six_month_change_str', 'N/A')
    ...
    # ── Cover stats: adapt for rental vs sale ────────────────────────────────
    if is_rent:
        stat1_value = _fmt_aed_short(annual_rent) if annual_rent else range_display
        stat1_label = 'ANNUAL RENT ESTIMATE'
        stat1_sub = confidence
        stat2_value = range_display
        stat2_label = 'RENT RANGE'
        stat2_sub = 'Yearly'
        monthly_low = data.get('monthly_rent_low')
        monthly_high = data.get('monthly_rent_high')
        stat3_value = f'{_fmt_aed_short(monthly_low)} – {_fmt_aed_short(monthly_high)}' if monthly_low else 'N/A'
        stat3_label = 'MONTHLY ESTIMATE'
        stat3_sub = 'Per month'
    else:
        stat1_value = _fmt_aed_short(val)
        stat1_label = 'ESTIMATED VALUE'
        stat1_sub = confidence
        stat2_value = change_str if change_str != 'N/A' else 'N/A'
        stat2_label = '6-MONTH GROWTH'
        stat2_sub = 'Price appreciation'
        if gross_yield:
    ...
    # Chart only meaningful for sale reports with value + change
    trend_svg = _make_trend_svg(val, change, range_low, range_high) if not is_rent else ''
  ...
  <div class="cover-body">
    ...
    <div class="cover-stats">
      <div class="cover-stat">
        <div class="cover-stat-value">{stat1_value}</div>
        <div class="cover-stat-label">{stat1_label}</div>
        <div class="cover-stat-sub">{stat1_sub}</div>
      </div>
      <div class="cover-stat">
        <div class="cover-stat-value {'positive' if change_positive else 'negative'}">{stat2_value}</div>
        <div class="cover-stat-label">{stat2_label}</div>
        <div class="cover-stat-sub">{stat2_sub}</div>
      </div>
      <div class="cover-stat">
...
<div class="page">
  ...
  <div class="page-content">
    ...
    <div class="valuation-hero">
      <div class="val-label">{'RENTAL ESTIMATE' if is_rent else 'RADIANT INTELLIGENCE™ ESTIMATE'}</div>
      <div class="val-amount">{_fmt_aed(annual_rent) if is_rent else _fmt_aed(val)}</div>
      <div class="val-meta">
        <div class="val-badge">{confidence}</div>
        {f'<div class="val-meta-item">Range <span>{range_display}</span></div>' if range_display != 'N/A' else ''}
        {f'<div class="val-meta-item">AED <span>{psf:,}/sqft</span></div>' if psf and not is_rent else ''}
        {f'<div class="val-change {chr(34)}{"positive" if change_positive else "negative"}{chr(34)}">' + ("▲" if change_positive else "▼") + f" {change_str} (6 months)</div>" if not is_rent and change_str != "N/A" else ""}
      </div>
    </div>
